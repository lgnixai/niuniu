...
use addon\fengchao\app\dict\config\PlatformDict;
use addon\fengchao\app\dict\delivery\KdniaoBrandDict;
use addon\fengchao\app\dict\delivery\YunjieBrandDict;
 ...
 * 聚合快递公共服务层
 * Class CommonService
 * @package addon\fengchao\service\core\common
 */
class CommonService extends BaseAdminService
{
    ...
    public function getBrand($platform,$delivery_type)
    {
        ...
        if($platform == 'yunjie'){
            return YunjieBrandDict::getBrand($delivery_type);
        }
    }
    /**
     ...
     */
    public function getSiteDriver( $type = '')
    {

        $info = (new CoreConfigService())->getConfig(0, PlatformDict::getType());
        if (empty($info)) {
    ...
    {
        $info = (new CoreConfigService())->getConfig($site_id, 'fengchao_CONFIG');
        if (empty($info)) {
    ...
    {
        $basedir = root_path() . 'addon' . DIRECTORY_SEPARATOR . 'fengchao' . DIRECTORY_SEPARATOR . 'movefile' . DIRECTORY_SEPARATOR;
