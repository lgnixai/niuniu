            ...
            (response: requestResponse) => {
				if (response.request.responseType != 'blob') {
					...
					if (res.ResultCode != 100) {
					    this.handleAuthError(res.Reason)
					    if (res.ResultCode != 401 && response.config.showErrorMessage !== false) this.showElMessage({ message: res.Reason||res.msg, type: 'error', dangerouslyUseHTMLString: true, duration: 5000 })
					    return Promise.reject(new Error(res.Reason || 'Error'))
					} else {
					    if (response.config.showSuccessMessage) ElMessage({ message: res.Reason, type: 'success' })
					    return res
					}
				}
